...
def print_menu():
    ...
    print("3. Вычитание в доп. коде (через отрицание и сложение)")
    print("4. Умножение в прямом коде")
    print("5. Деление в прямом коде (Фиксированная точка 26.5)")
...
def main():
    while True:
        ...
        try:
            if choice == '0':
                print("Завершение работы эмулятора АЛУ.")
                sys.exit(0)
                ...
                if v2 == 0:
                    print("Ошибка: Деление на ноль запрещено логикой АЛУ.")
                    continue
                a, b = to_direct(v1), to_direct(v2)
                res = divide_binary(a, b)
                print(f"\nДелимое:       {format_bin(a)} ({v1})")
                print(f"Делитель:      {format_bin(b)} ({v2})")
                print(f"Частное (bin): {format_bin(res)} (Формат 26.5)")
                ...
                if v1 < 0 or v2 < 0:
                    print(
                        "Внимание:"
                        " Gray BCD в нашей реализации работает только с беззнаковыми числами."
                    )
                    continue
